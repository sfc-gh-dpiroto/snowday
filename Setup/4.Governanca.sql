...
CREATE OR REPLACE ROLE ANALISTA_JR;

USE ROLE ACCOUNTADMIN;

begin

    GRANT ALL ON WAREHOUSE COMPUTE_WH TO ROLE ANALISTA_JR;
    GRANT USAGE ON DATABASE tb_101 TO ROLE ANALISTA_JR;
    GRANT USAGE ON ALL SCHEMAS IN DATABASE tb_101 TO ROLE ANALISTA_JR;
    GRANT SELECT ON ALL TABLES IN SCHEMA tb_101.raw_customer TO ROLE ANALISTA_JR;
    GRANT SELECT ON ALL TABLES IN SCHEMA tb_101.raw_pos TO ROLE ANALISTA_JR;
    GRANT SELECT ON ALL VIEWS IN SCHEMA tb_101.analytics TO ROLE ANALISTA_JR;
end;
...
CREATE OR REPLACE MASKING POLICY PUBLIC.PII_MASK AS (val STRING) RETURNS STRING ->
        CASE 
...
ALTER TAG DADO_SENSIVEL SET MASKING POLICY PUBLIC.PII_MASK;
...
INSERT INTO PUBLIC.row_policy_map
    VALUES ('ANALISTA_JR','Rio de Janeiro'); 
...
DROP ROLE IF EXISTS ANALISTA_JR;

-- unset our Masking Policies
ALTER TAG PII unSET MASKING POLICY PUBLIC.PII_MASK;
DROP TAG PUBLIC.PII ;
drop MASKING POLICY PUBLIC.PII_MASK;
